...
export default function FlashSalesTable({ productId, revision }: { productId: string; revision: number }) {
  ...
  const loading = Boolean(selectedProductId) && state.loadedRevision !== requestRevision
  ...
  useEffect(() => {
    ...
    async function loadSales() {
      try {
        const response = await fetch(`/api/products/${encodeURIComponent(selectedProductId)}/flash-sales`, { signal: controller.signal })
        if (!response.ok) {
    ...
    <section className="sales-section" aria-labelledby="sales-heading">
      <div className="sales-heading">
        ...
        <div className="sales-controls">
          <label>
            Product ID
            <input value={selectedProductId} onChange={(event) => setSelectedProductId(event.target.value)} />
          </label>
          <button type="button" aria-label="Refresh flash sales" onClick={() => setRefresh((value) => value + 1)}>{loading ? 'Refreshing…' : 'Refresh'}</button>
        </div>
      </div>
      <p className="hint">{selectedProductId ? `Product: ${selectedProductId}` : 'Enter a product ID to see its flash sales.'}</p>
      ...
      <div className="table-scroll">
        <table aria-busy={loading}>
          ...
          <tbody>
            ...
            {state.sales.length === 0 && <tr><td colSpan={4} className="empty-sales">{!selectedProductId ? 'Enter a product ID to load flash sales.' : loading ? 'Loading…' : state.error ? 'Flash sales could not be loaded. Try Refresh.' : 'No flash sales yet. Create one using the form above.'}</td></tr>}
          </tbody>
        </table>
      </div>
    </section>
